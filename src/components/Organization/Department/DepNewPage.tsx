...
import { createDep } from '../../../lib/api/organization';
...
const DepNewPage = (props: Props) => {
  ...
  const saveDep = async () => {
    try {
      const res = await createDep({code: code, name: name})
      if (res.data.status === 500) {
        setErr({severity: "error", message: "事業部情報登録エラー(500)"});
      } else {
        props.close(true);
        setCode("");
        setName("");
      ...
      setErr({severity: "error", message: "事業部情報登録エラー"});
    }
    setIsLoading(false);
  }
        ...
        <Box component='div' sx={{ backgroundColor: '#fff', height: '100%', border: "0.5px solid #000", boxShadow: "1px 1px 2px rgba(0, 0, 0, 0.5)" }}>
          <AppBar position='static'>
            <Toolbar variant="dense">
              <Typography variant='caption' component="div" sx={{ flexGrow: 1, fontSize: cmnProps.topFontSize, fontFamily: cmnProps.fontFamily }}>事業部新規作成</Typography>
              <IconButton
                size="medium"
                edge="end"
                color='inherit'
