...
import { getDiv, updateDiv } from '../../../lib/api/organization';
...
const initData = {depCode: "", depName: "", code: "", name: ""};
const DivUpdPage = (props: Props) => {
  ...
  // 初期処理
  useEffect(() => {
    if (!isEmpty(props.divId)) {
      // パラメータ設定ありの場合のみ課情報取得
      setIsLoading(true);
      handleGetDiv(props.divId);
    } 
  }, [props.divId]);

  // 課情報取得
  const handleGetDiv = async (id: number | null) => {
    try {
      const res = await getDiv(id);
      setData({
        depCode: res.data.div.depCode,
        depName: res.data.div.depName,
        code: res.data.div.code,
        name: res.data.div.name,
      });
      setCode(res.data.div.code);
      setName(res.data.div.name);
    } catch (e) {
  ...
  const saveDiv =async () => {
    try {
      const res = await updateDiv(props.divId, {code: code, name: name})
      if (res.data.status === 500) {
        setErr({severity: "error", message: "課情報更新エラー(500)"});
      } else {
        props.close(true);
        setCode("");
        setName("");
      ...
      setErr({severity: "error", message: "課情報更新エラー"});
    }
    setIsLoading(false);
  }
        ...
        <Box component='div' sx={{ backgroundColor: '#fff', height: '100%', border: "0.5px solid #000", boxShadow: "1px 1px 2px rgba(0, 0, 0, 0.5)" }}>
          <AppBar position='static'>
            <Toolbar variant="dense">
              <Typography variant='caption' component="div" sx={{ flexGrow: 1, fontSize: cmnProps.topFontSize, fontFamily: cmnProps.fontFamily }}>課変更</Typography>
              <IconButton
                size="medium"
                edge="end"
                color='inherit'
          ...
          <Box component="div" sx={{ marginX: "20px", mt: 4 }}>
            <Typography variant='caption' component="div" color="primary.main" sx={{ fontSize: cmnProps.fontSize, fontFamily: cmnProps.fontFamily }}>{"[上位部門] " + data.depCode + ": " + data.depName}</Typography>
          </Box>
