...
const EmpAllPage = (props: Props) => {
  ...
  // 所属名編集
  const setOrgName = (e: Emp) => {
    if (isEmpty(e.divisionId)) {
      return "";
    } else {
      if (e.divCode==="dep") {
        return e.depName;
      } else {
        return e.depName + "　" + e.divName;
      }
    }
  }
        ...
        <Box component='div' sx={{ backgroundColor: '#fff', height: '100%', border: "0.5px solid #000", boxShadow: "1px 1px 2px rgba(0, 0, 0, 0.5)" }}>
          <AppBar position='static'>
            <Toolbar variant="dense">
              <Typography variant='caption' component="div" sx={{ flexGrow: 1, fontSize: cmnProps.topFontSize, fontFamily: cmnProps.fontFamily }}>全ての社員</Typography>
              <IconButton
                size="medium"
                edge="end"
                color='inherit'
          ...
          <TableContainer component={Paper} sx={{ maxHeight: err.severity ? 'calc(100% - 150px)' : 'calc(100% - 90px)', m: '20px', px: '20px', py: '10px', width: 'auto' }}>
            <Table sx={{ width: 850 }} stickyHeader aria-label="employees table">
              <TableHead>
                <TableRow>
                  <CustomCell sx={{ width: 100 }}>社員番号</CustomCell>
                  <CustomCell sx={{ width: 200 }}>氏名</CustomCell>
                  <CustomCell sx={{ width: 300 }}>所属</CustomCell>
              ...
              <TableBody>
                { emps.map((e,i) => (
                  ...
                  >
                    ...
                    <CustomCell>{setOrgName(e)}</CustomCell>
                    <CustomCell>
