...
import { getSalesActions, updateSalesActions, updateActionParams } from '../../lib/api/prospect';
...
const SalesActionPage = (props: Props) => {
  const [err, setErr] = useState<AlertType>({ severity: null, message: "" });
  const [data, setData] = useState<updateActionParams>({salesactions: []});
  ...
  const handleGetActions = async () => {
    ...
    try {
      ...
      setData({
        salesactions: res.data.salesactions});
    } catch (e) {
      setErr({severity: "error", message: "営業アクション取得エラー"});
    } 
    setIsLoading(false);
  }
  ...
  const handleAdd = () => {
    setData({ ...data,
      salesactions : [...data.salesactions,
      {id: null, 
      name: "", 
      colorR: 51,
      colorG: 51,
  ...
  const handleChangeName= (i: number, value: string) => {
    const tmpActions = [...data.salesactions];
    tmpActions[i].name = value;
    setData({
      ...data,
      salesactions: tmpActions
    });
  }
  ...
  const handleMoveUp = (i: number) => {
    if (i !== 0) {
      let _tmpActions = data.salesactions.slice(0, i-1);
      let _tmpAction = data.salesactions[i-1];
      let tmpAction = data.salesactions[i];
      let tmpActions_ = data.salesactions.slice(i+1);

      let tmpActions = _tmpActions.concat(tmpAction, _tmpAction, tmpActions_);
      setData({
        ...data,
        salesactions: tmpActions
      });
  ...
  const handleMoveDown = (i: number) => {
    if (i !== (data.salesactions.length - 1)) {
      let _tmpActions = data.salesactions.slice(0, i);
      let tmpAction = data.salesactions[i];
      let tmpAction_ = data.salesactions[i+1];
      let tmpActions_ = data.salesactions.slice(i+2);

      let tmpActions = _tmpActions.concat(tmpAction_, tmpAction, tmpActions_);
      setData({
        ...data,
        salesactions: tmpActions
      });
  ...
  const handleDelete = (i: number) => {
    const tmpActions = [...data.salesactions];
    tmpActions[i].del = !tmpActions[i].del;
    setData({
      ...data,
      salesactions: tmpActions
    });
  ...
  // 保存ボタン活性判断
  const setDisabled = () => {
    if(!data.salesactions.length){
      return true;
    }
    let ret: boolean = false;
    data.salesactions.forEach(s => {
      if(!s.del){
        if((isEmpty(s.name))){
          ret = true;
        }
  ...
  const saveActions = async () => {
    try {
      const res = await updateSalesActions(data);
      if (res.data.status === 500) {
        setErr({severity: "error", message: "営業アクション保存エラー(500)"});
      } else {
        props.close();
        setData({salesactions: []});
        setErr({severity: null, message: ""});
      ...
      setErr({severity: "error", message: "営業アクション保存エラー"});
    }
    setIsLoading(false);
  }
  ...
  const handleChangeColor = (i: number, colorR: number, colorG: number, colorB: number, colorA: number) => {
    const tmpActions = [...data.salesactions];
    tmpActions[i].colorR = colorR;
    tmpActions[i].colorG = colorG;
    tmpActions[i].colorB = colorB;
    tmpActions[i].colorA = colorA;
    setData({
      ...data,
      salesactions: tmpActions
    });    
          ...
          <Box component='div' sx={{ height: '100vh', width: '100vw', backgroundColor: '#fff'}}>
            <AppBar position='static'>
              <Toolbar variant="dense">
                <SettingsSuggestIcon />
                <Typography variant='caption' component="div" sx={{ ml: 1, flexGrow: 1, fontSize: cmnProps.topFontSize, fontFamily: cmnProps.fontFamily }}>Sales Action 〜営業アクション〜</Typography>
                <IconButton
                  size="medium"
                  edge="end"
                  color='inherit'
            ...
            <Box component='div' sx={{ mx: 5 }}>
              ...
              <TableContainer component={Paper}>
                <Table sx={{ width: 700 }} stickyHeader aria-label="salesaction table">
                  <TableHead>
                    <TableRow>
                      <CustomHead sx={{ width: 400 }}>営業アクション</CustomHead>
                      <CustomHead sx={{ width: 100 }}>グラフ配色</CustomHead>
                      <CustomHead sx={{ width: 200 }} />
                    </TableRow>
                  </TableHead>
                  <TableBody>
                    { data.salesactions.map((s,i) => (
                      <TableRow
                        key={i}
                      ...
                      >
                        ...
                        <CustomCell>
                          <ColorPicker colorR={s.colorR} colorG={s.colorG} colorB={s.colorB} colorA={s.colorA} i={i} handleChange={handleChangeColor} />
                        </CustomCell>
                        <CustomCell>
                          ...
                          <IconButton aria-label="move-down" onClick={() => handleMoveDown(i)} disabled={(i === (data.salesactions.length-1)) ? true : false }>
                            {(i === (data.salesactions.length-1)) ? (
...
export default SalesActionPage;
